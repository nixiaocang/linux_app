...
class FuploadHelper2(object):
	...
	print 
        ...
        for tbinfo in tb_res:
            ...
            remotepath = '/tmp/%s/%s' % (ds_id, tbinfo['tb_id'])
    ...
    def check(self, ds_id, tb_id):
        ...
        ezio_check = self.conf.get('server', 'Ezio') + '/new/newcheck'
    ...
    def split_file(self, queue, sub_path, err=None):
        ...
        while 1:
            ...
            if not chunk:
                break
            partnum += 1
            filename = os.path.join(sub_path, ('all.zip-%s' % partnum))
    ...
    def send_file(self, queue, remotepath):
        ...
        while True:
            ...
            if info:
                ...
                sconf = self.conf.get_section('sftp')
                bsftp = BdpSftpClient(sconf['host'],int(sconf['port']),sconf['username'],sconf['password'])
                rpath = os.path.join(remotepath, 'all.zip-%s' % info['partnum'])
                bsftp.sftp_put(info['das'], rpath)
                bsftp.close()
...
if __name__=='__main__':
    ...
    username = "jiaoguofu"
    password = "jiao1993"
    local_path = "/Users/jiaoguofu/Desktop/fsplit_bak/tb"
    ds_name = "fsplit_bak"
    tbname = "tb"
    import datetime
